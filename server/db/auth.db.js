...
const isValidTokenDb = async ({ token, email, curDate }) => {
  const { rows } = await pool.query(
    `
      SELECT EXISTS(select * from pern."resetTokens" 
...
const createResetTokenDb = async ({ email, expireDate, fpSalt }) => {
  await pool.query(
    'insert into pern."resetTokens" (email, expiration, token) values ($1, $2, $3)',
...
const setTokenStatusDb = async (email) => {
  await pool.query(
    'update pern."resetTokens" set used = $1 where email = $2',
    [true, email]
  );
...
const deleteResetTokenDb = async (curDate) => {
  await pool.query('delete from pern."resetTokens" where expiration <= $1', [
    curDate,
  ]);
  return true;
};
